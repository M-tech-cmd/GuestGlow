...
const ListRoom = () => {
  ...
  return (
    <div>
      ...
      subTitle='View,edit and manage all listed rooms. Keep the infor,mation up-to-date to provide the latest experience for users'/>
            ...
            
            {
              rooms.map((item , index)=>(
                <tr key={index}>
                  ...
                  {/* jon all ammenities by a coma (, ) */}
                  <td className='py-3 px-4 text-gray-700 border-t border-gray-300 max-sm:hidden'>{item.amenities.join(', ')}</td>
                  <td className='py-3 px-4 text-gray-700 border-t border-gray-300  '>Ksh. {item.pricePerNight}</td>
